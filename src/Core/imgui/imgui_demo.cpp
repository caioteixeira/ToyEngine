...
void ImGui::ShowTestWindow(bool* p_open)
{
    ...
    if (ImGui::CollapsingHeader("Widgets"))
    {
        ...
        if (ImGui::TreeNode("Images"))
        {
            ...
            if (ImGui::IsItemHovered())
            {
                ...
                ImGui::Image(tex_id, ImVec2(128, 128), uv0, uv1, ImColor(255, 255, 255, 255),
                             ImColor(255, 255, 255, 128));
                ImGui::EndTooltip();
            }
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                if (ImGui::ImageButton(tex_id, ImVec2(32, 32), ImVec2(0, 0), ImVec2(32.0f / tex_w, 32 / tex_h),
        ...
        ImGui::SameLine();
        ShowHelpMarker(
            "Click on the colored square to change edit mode.\nCTRL+click on individual component to input value.\n");
    ...
    if (ImGui::CollapsingHeader("Keyboard, Mouse & Focus"))
    {
        ...
        if (ImGui::TreeNode("Keyboard & Mouse State"))
        {
            ...
            ImGui::Text("KeyMods: %s%s%s%s", io.KeyCtrl ? "CTRL " : "", io.KeyShift ? "SHIFT " : "",
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref)
{
    ...
    if (ImGui::TreeNode("Colors"))
    {
        ...
        if (ImGui::Button("Copy Colors"))
        {
            ...
            for (int i = 0; i < ImGuiCol_COUNT; i++)
            {
                ...
                if (!output_only_modified || memcmp(&col, (ref ? &ref->Colors[i] : &default_style.Colors[i]),
                                                    sizeof(ImVec4)) != 0)
                    ImGui::LogText("style.Colors[ImGuiCol_%s]%*s= ImVec4(%.2ff, %.2ff, %.2ff, %.2ff);" IM_NEWLINE, name,
                                   22 - (int)strlen(name), "", col.x, col.y, col.z, col.w);
            }
            ImGui::LogFinish();
        }
        ImGui::SameLine();
    ...
    if (ImGui::TreeNode("Fonts", "Fonts (%d)", ImGui::GetIO().Fonts->Fonts.Size))
    {
        ...
        for (int i = 0; i < atlas->Fonts.Size; i++)
        {
            ...
            ImGui::BulletText("Font %d: \'%s\', %.2f px, %d glyphs", i,
                ...
                if (ImGui::TreeNode("Glyphs", "Glyphs (%d)", font->Glyphs.Size))
                {
                    ...
                    for (int base = 0; base < 0x10000; base += 256)
                    {
                                                         ...
                                                         base + 255, count, count > 1 ? "glyphs" : "glyph"))
                        {
                            ...
                            for (int n = 0; n < 256; n++)
                            {
                                ImVec2 cell_p1(base_pos.x + (n % 16) * (cell_size.x + cell_spacing),
                                ...
                                font->RenderChar(draw_list, cell_size.x, cell_p1, ImGui::GetColorU32(ImGuiCol_Text),
                                                 (ImWchar)(base + n));
...
static void ShowExampleAppConstrainedResize(bool* p_open)
{
    ...
    if (type == 4) ImGui::SetNextWindowSizeConstraints(ImVec2(0, 0), ImVec2(FLT_MAX, FLT_MAX),
                                                       CustomConstraints::Square); // Always Square
    if (type == 5) ImGui::SetNextWindowSizeConstraints(ImVec2(0, 0), ImVec2(FLT_MAX, FLT_MAX), CustomConstraints::Step,
                                                       (void*)100); // Fixed Step
...
static void ShowExampleAppCustomRendering(bool* p_open)
{
    ...
    ImGui::Separator();
    {
        ...
        draw_list->AddRectFilledMultiColor(
            canvas_pos, ImVec2(canvas_pos.x + canvas_size.x, canvas_pos.y + canvas_size.y), ImColor(50, 50, 50),
            ImColor(50, 50, 60), ImColor(60, 60, 70), ImColor(50, 50, 60));
...
struct ExampleAppConsole
{
    ...
    void Draw(const char* title, bool* p_open)
    {
        ...
        for (int i = 0; i < Items.Size; i++)
        {
            ...
            // A better implementation may store a type per-item. For the sample let's just parse the text.
        ...
        if (ImGui::IsItemHovered() || (ImGui::IsRootWindowOrAnyChildFocused() && !ImGui::IsAnyItemActive() && !ImGui::
            IsMouseClicked(0)))
